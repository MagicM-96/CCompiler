...
identifierDeclaration
     : ID BRACKET_OPEN NUM BRACKET_CLOSE {push(&programstack,$1);$$.val=$3;char* temp=(char*)malloc(sizeof(char)*10);sprintf(temp,"%s [%d]",$1,$3);$$.var = temp;}
     | ID {push(&programstack,$1);$$.val=1;$$.var=$1;}
     ;
...
functionParameterList
     : functionParameter {$$.val=1;$$.var=$1;}
     | functionParameterList COMMA functionParameter {$$.val=$1.val+1;char* temp=(char*)malloc(sizeof(char)*100);sprintf(temp,"%s, %s",$1.var,$3);$$.var=temp;}
     ;
	
functionParameter
     : type identifierDeclaration {push(&programstack,$1.type);char* temp=(char*)malloc(sizeof(char)*20);sprintf(temp,"%d",$2.val);push(&programstack,temp);sprintf(temp,"int %s",$2.var);$$=temp;}
     ;
...
expression
     ...
     | expression PLUS expression   {getScannedLines(); if(isTypeCompatible($1.type, $3.type)){$$.type="INT";char* temp=(char*)malloc(sizeof(char)*4);addCode(OPADD,&temp,$1.var,$3.var,NULL);$$.var=temp;} else {throwMathOpError(errorLineInfo);};}
     | expression MINUS expression  {getScannedLines(); if(isTypeCompatible($1.type, $3.type)){$$.type="INT";char* temp=(char*)malloc(sizeof(char)*4);addCode(OPSUB,&temp,$1.var,$3.var,NULL);$$.var=temp;} else {throwMathOpError(errorLineInfo);};}
     | expression MUL expression    {getScannedLines(); if(isTypeCompatible($1.type, $3.type)){$$.type="INT";char* temp=(char*)malloc(sizeof(char)*4);addCode(OPMUL,&temp,$1.var,$3.var,NULL);$$.var=temp;} else {throwMathOpError(errorLineInfo);};}
     | expression DIV expression    {getScannedLines(); if(isTypeCompatible($1.type, $3.type)){$$.type="INT";char* temp=(char*)malloc(sizeof(char)*4);addCode(OPDIV,&temp,$1.var,$3.var,NULL);$$.var=temp;} else {throwMathOpError(errorLineInfo);};}
